...
const TestResultsPanel = ({ onCheckClick }) => {
    ...
    try {
      ...
      if (parsed && parsed.servers && Array.isArray(parsed.servers)) {
        const serverList = parsed.servers.map(server => ({
          name: server.server || server.name || es.testPanel.unnamedServer,
          ...server
        }));
  ...
  const renderHelp = () => {
      ...
      return (
        <div className="mt-4 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
          <div className="flex items-start justify-between mb-2">
            <h4 className="text-sm font-semibold text-blue-900 dark:text-blue-200">{es.testPanel.helpTitle}</h4>
            <button
              onClick={() => setShowHelp(false)}
              className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-200"
              title={es.testPanel.hideHelpTitle}
            >
    ...
    return (
      <div className="mt-4 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
        <div className="flex items-start justify-between mb-2">
          <h4 className="text-sm font-semibold text-blue-900 dark:text-blue-200">{es.testPanel.cliHelpTitle}</h4>
          <button
            onClick={() => setShowHelp(false)}
            className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-200"
            title={es.testPanel.hideHelpTitle}
        ...
        <div className="text-sm text-blue-800 dark:text-blue-300 space-y-2">
          <p>
            ...
            {es.testPanel.cliHelpP1After}
          </p>
          <div className="bg-gray-900 text-gray-100 p-2 rounded font-mono text-xs">
            $ datacontract api
          </div>
          <p>
            {es.testPanel.cliHelpP2Before} <span className="font-mono">{apiServerUrl}</span>.
          </p>
          <p>
            {es.testPanel.cliHelpP3Before}{' '}
            <a
      ...
      return (
        <div className="space-y-3">
          <div className="text-sm text-red-700 dark:text-red-300">
            <p className="font-semibold">{es.testPanel.errorLabel}</p>
        ...
        return (
          <div className="space-y-3">
            ...
            {data.logs && data.logs.length > 0 && (
              <div className="mt-4">
                <h4 className="text-sm font-semibold mb-2 text-gray-700 dark:text-gray-300">{es.common.logs}</h4>
      ...
      <div className="h-full bg-gray-50 dark:bg-gray-900 flex flex-col overflow-hidden">
        ...
        <div className="p-4 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-lg font-bold text-gray-800 dark:text-gray-200">{es.testPanel.title}</h2>
            {lastTestStatus && (
              <div className="flex items-center gap-1.5">
                {lastTestStatus.success ? (
                  <>
                    <span className="text-sm text-gray-700 dark:text-gray-300">{es.testPanel.passed}</span>
                    <span className="inline-flex items-center text-green-600" title={es.testPanel.allPassedTitle}>
                  ...
                  <>
                    <span className="text-sm text-gray-700 dark:text-gray-300">{es.testPanel.failedTests}</span>
            ...
            <span className="inline-flex w-full">
              <div className="-mr-px grid grid-cols-1 flex-1">
                <select
                  ...
                  aria-label={es.common.selectServer}
          ...
          <div className="mt-2 flex items-center justify-between gap-2 min-w-0">
            <div className="text-xs text-gray-500 dark:text-gray-400 min-w-0 truncate">
              {es.testPanel.runTestsVia}{' '}
              <a href={apiServerUrl} target="_blank" rel="noopener noreferrer" className="font-mono text-blue-600 dark:text-blue-400 hover:underline" title={apiServerUrl}>{apiServerUrl}</a>
            </div>
            <div className="flex items-center gap-2 shrink-0">
              ...
              >
                {showHelp ? es.common.hideHelp : es.common.showHelp}
              </button>
            </div>
          </div>
          {showHelp && renderHelp()}
        ...
        <div className="flex-1 flex items-center justify-center text-gray-500 dark:text-gray-400">
          <div className="text-center">
            ...
            <h3 className="mt-2 text-sm font-medium">{es.testPanel.noResultsTitle}</h3>
            <p className="mt-1 text-sm">{es.testPanel.noResultsBody}</p>
          </div>
        </div>
        ...
        <div className="flex gap-2 items-center">
          ...
          {currentResult && (
            <button
              ...
              title={es.testPanel.clearResultsTitle}
            >
              {es.common.clear}
            </button>
          )}
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex flex-col">
                  <span className={`text-sm font-semibold ${
                    ...
                    {currentResult.success ? es.common.passed : es.common.failed}
                  </span>
                  {currentResult.data && currentResult.data.server && (
                    <span className="text-xs text-gray-600 dark:text-gray-400">
                      {es.testPanel.serverLabel} {currentResult.data.server}
                    </span>
                  )}
                </div>
              </div>
