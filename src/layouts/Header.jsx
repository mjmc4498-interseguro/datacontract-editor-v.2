...
const Header = () => {
	...
	const handleClose = () => {
		if (window.confirm(es.header.confirmLeave)) {
	...
	const handleSave = async () => {
		...
		try {
			...
			alert(es.header.saveFailed(error.message));
		}
	};
	...
	const handleDelete = () => {
		if (window.confirm(es.header.confirmDelete)) {
	...
	const handleNew = () => {
		if (window.confirm(es.header.confirmNew)) {
	...
	const handleExample = () => {
		if (window.confirm(es.header.confirmExample)) {
			const store = useEditorStore.getState();
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" version="1.1">
  <path
		...
		stroke="none" fill="currentColor" fillRule="evenodd" strokeWidth="0.024"></path>
                    ...
                    <span className="hidden lg:inline">{es.header.preview}</span>
                </button>
                <button type="button"
												onClick={toggleWarnings}
												title={es.header.validation}
                    ...
                    <span className="inline-flex items-center justify-center">
                      ...
                      <span className="hidden lg:inline">{es.header.validation}</span>
											{totalCount === 0 ? (
												<span className="lg:ml-1 inline-flex items-center text-green-600" title={es.header.valid}>
                    ...
                    <span className="inline-flex items-center justify-center">
                      ...
                      <span className="hidden lg:inline">{es.header.tests}</span>
											{testResults.length > 0 && (
												hasTestPassed ? (
													<span className="lg:ml-1.5 inline-flex items-center text-green-600" title={es.header.testsPassed}>
                            <div className="size-4">
                              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                <path fillRule="evenodd"
																			d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z"
                            ...
                            {testProblemCount}
													...
													<span className="lg:ml-1.5 inline-flex items-center text-red-600" title={es.header.testsFailed}>
                            <div className="size-4">
                              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                <path fillRule="evenodd"
							...
							{isEmbeddedMode ? (
								...
								<>
									...
									<button
										...
										onClick={handleSave}
									>
										{editorConfig?.saveLabel || es.common.save}
									</button>
								</>
							) : (
			...
			<Modal
				...
				confirmText={es.header.goToYamlView}
				onCancel={() => setShowParseErrorModal(false)}
				cancelText={es.common.close}
			>
				<div className="flex gap-4">
					...
					<div className="min-w-0">
						<h3 className="text-base font-semibold text-gray-900">{es.header.cannotSaveTitle}</h3>
						<p className="mt-1 text-sm text-gray-500">
							{es.header.cannotSaveBody}
						</p>
						{yamlParseError && (
