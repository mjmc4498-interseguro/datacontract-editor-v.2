...
import { ASSISTANT_SYSTEM } from '../locale/assistantPrompt.es.js';
import { es } from '../locale/es.js';

/** Registra las herramientas integradas al cargar el módulo */
registerBuiltInTools();

function buildSystemPrompt(contract) {
	let prompt = ASSISTANT_SYSTEM;
	if (contract) {
		prompt += `\n\n${es.ai.currentContractLabel}\n\`\`\`yaml\n${contract}\n\`\`\``;
	}
	return prompt;
}

/** Icono de chispas para la IA */
...
/** Extrae bloques [[OPTIONS]] de la respuesta del modelo */
...
/** Botones para opciones sugeridas por el modelo */
...
/** Icono de documento para llamadas a herramientas */
...
export default function AiChat() {
	...
	const sendMessage = useCallback(async (userMessage) => {
		...
		if (!isAiConfigured) {
			setError(new Error(es.ai.aiNotConfiguredError));
			return;
		}

		setError(null);
		setIsLoading(true);
		setStatusMessage(es.ai.thinking);
		...
		try {
			...
			const result = await chatWithTools({
				...
				tools: [], // aiService añade las herramientas registradas automáticamente
				config: {
				...
				callbacks: {
					onContent: (chunk, content) => {
						setStatusMessage(es.ai.generating);
					...
					onToolCallsStart: (toolCalls) => {
						...
						if (names.length === 1) {
							setStatusMessage(es.ai.callingTool(names[0]));
						} else if (names.length > 1) {
							setStatusMessage(es.ai.callingTools(names.length));
						}
					},
						...
						for (let i = 0; i < toolCalls.length; i++) {
							...
							if (name === 'updateContract' && parsedResult.updatedYaml) {
								setPendingAiChange({
									...
									summary: parsedResult.summary || es.ai.suggestedChangesFallback,
			...
			if (e.name !== 'AbortError') {
				setError(e);
				console.error('Error en el chat de IA:', e);
			}
		} finally {
			setIsLoading(false);
	...
	if (!isAiConfigured) {
		...
		let hint;
		if (isAnthropic) {
			hint = es.ai.hintAnthropic;
		} else if (isOpenRouter) {
			hint = es.ai.hintOpenRouter;
		} else {
			hint = es.ai.hintOpenAI;
		}
		return (
			<div className="flex flex-col h-full">
				<div className="flex-1 flex items-center justify-center p-4">
					<div className="text-center max-w-sm">
						...
						<h3 className="text-sm font-medium text-gray-900 mb-2">{es.ai.notConfiguredTitle}</h3>
						...
						<div className="bg-gray-50 rounded-lg p-3 text-left text-xs font-mono text-gray-600">
							{isAnthropic ? (
								...
								<>
									<div><span className="text-gray-400">provider:</span> openrouter</div>
									<div><span className="text-gray-400">endpoint:</span> https://openrouter.ai/api/v1/chat/completions</div>
									<div><span className="text-gray-400">apiKey:</span> sk-or-v1-...</div>
									<div><span className="text-gray-400">model:</span> openrouter/auto</div>
								</>
							) : (
								<>
			...
			<div className="flex-1 overflow-y-auto overflow-x-hidden p-4 space-y-4 min-h-0">
				{messages.length === 0 ? (
					<div className="flex flex-col items-center justify-center h-full text-center">
						...
						<h3 className="text-sm font-medium text-gray-900 mb-2">{es.ai.assistantTitle}</h3>
						<p className="text-xs text-gray-500 mb-6 max-w-xs">
							{es.ai.emptyStateSubtitle}
						</p>
						<div className="space-y-2 w-full max-w-xs">
							{es.ai.suggestedPrompts.map((prompt, index) => (
								<button
									key={index}
											...
											<div className="flex items-center justify-between">
												<div className="flex items-center gap-2 text-indigo-700">
													...
													<span className="font-medium">
														{message.toolCall.name === 'updateContract' ? es.ai.contractUpdate : message.toolCall.name}
													</span>
												</div>
												{lastAppliedAiChange && (
													<button
			...
			<div className="border-t border-gray-200 p-4">
				<form onSubmit={handleSubmit} className="flex gap-2">
					<input
						...
						placeholder={isPromptReady ? es.ai.askPlaceholder : es.ai.loadingField}
