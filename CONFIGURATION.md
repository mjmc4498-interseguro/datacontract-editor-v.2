...
VITE_AI_PROVIDER=openai           # 'openai', 'openrouter' or 'anthropic'
...
> **OpenRouter quick start:** set `VITE_AI_PROVIDER=openrouter` and `VITE_AI_API_KEY=sk-or-v1-...`. The endpoint (`https://openrouter.ai/api/v1/chat/completions`) and model (`openrouter/auto`) are filled in automatically; you can still override `VITE_AI_MODEL` (e.g. `openai/gpt-4o`, `anthropic/claude-3.5-sonnet`). The editor is also auto-detected as OpenRouter whenever the endpoint host is `openrouter.ai`.
...
| `AI_PROVIDER` | `openai` | `openai`, `openrouter` or `anthropic` |
| `AI_ENDPOINT` | - | API endpoint URL (optional for `openrouter`) |
...
**OpenRouter Docker example:**

```bash
docker run -p 4173:4173 \
  -e AI_ENABLED=true \
  -e AI_PROVIDER=openrouter \
  -e AI_API_KEY=sk-or-v1-xxx \
  -e AI_MODEL=openrouter/auto \
  datacontract-editor
```
...
ai: {
  ...
  endpoint: 'https://...',          // API endpoint (required, auto-filled for 'openrouter')
  ...
  siteUrl: 'https://my-app.example',// (openrouter only) sent as HTTP-Referer
  siteName: 'My App',               // (openrouter only) sent as X-Title
...
|----------|------------|---------------|----------|
| OpenAI (and generic compatible) | `openai` | `gpt-4o` | `https://api.openai.com/v1/chat/completions` |
| OpenRouter | `openrouter` | `openrouter/auto` | `https://openrouter.ai/api/v1/chat/completions` (default) |
| Anthropic | `anthropic` | `claude-sonnet-4-20250514` | `https://api.anthropic.com/v1/messages` |

The `openai` provider works with any OpenAI-compatible API: OpenAI, Azure OpenAI, Ollama, LiteLLM, vLLM, etc.

The `openrouter` provider reuses the OpenAI-compatible wire format but adds a few conveniences:

- `endpoint` and `model` default automatically to OpenRouter's values when not specified.
- The editor is auto-detected as OpenRouter if the endpoint host is `openrouter.ai`, so setting `provider: 'openrouter'` is optional when the endpoint is explicit.
- The `HTTP-Referer` and `X-Title` headers recommended by OpenRouter are added automatically (use `siteUrl` / `siteName` to customize; explicit `headers` still win).
- Any OpenRouter model slug is accepted (`openrouter/auto`, `openai/gpt-4o`, `anthropic/claude-3.5-sonnet`, `google/gemini-2.0-flash`, ...).
...
| OpenRouter | `bearer` | `Authorization: Bearer <key>` |
...
init({
  ...
  ai: {
    ...
    provider: 'openrouter',
    apiKey: 'sk-or-v1-xxx',
    // endpoint and model default to OpenRouter if omitted
